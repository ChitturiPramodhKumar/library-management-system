...
class InventoryDBHandler(MySQLDBHandler)  :
    ...
    def get_borrowed_books(self, person, contact, result_dict = ''):
        query = borrow_book_by_person(person, contact)
    ...
    def search(self, search_line, _filter, table_name, person, contact):
        try:
            data = []
            if _filter == 'book_name':
                query = search_by_book_name(search_line, table_name, person, contact)
                data = self.fetch(query, result_dict = True)
            elif _filter == 'book_id':
                query = search_by_book_id(search_line, table_name, person, contact)
                data = self.fetch(query, result_dict = True)
            elif _filter == 'borrow_id':
                query = search_by_borrow_id(search_line, person, contact)
                data = self.fetch(query, result_dict = True)
            return data
    ...
    def add_book_to_inventory(self, book_name, author, edition, category, description):
        ...
        if not book_exists:
            ...
            date = dateandtime.strftime('%d %B,%Y')
    ...
    def add_book_to_borrow(self, person, contact, book_id):
        book_exists, record= self.is_book_exists_byid(book_id)
        print(record)
        count = record['Count']
        ...
        if book_exists:
            if availability == 'YES' and books_available > 0:
                ...
                if books_available > 1:
                    ...
                    condition = f"id = '{book_id}'"
                self.update_table(INVENTORY_TABLE, changes, condition)
                borrow_record = (borrow_id, person, contact, book_id, book_name, author, book_edition, borrowed_at, 'ACTIVE', '-')
                self.insert_into_borrow(borrow_record)
        return borrowed
    
    def is_borrowed(self, person, contact, book_id):
        is_borrowed = False
        ...
        for item in data:
            if item['Person'] == person and item['Contact number'] == item['Book ID'] == book_id:
                is_borrowed = True
                record = item
    ...
    def report(self):
        weekly_data = ''
        monthly_data = ''
        inventory_report = REPORT_FROM_INVENTORY
        data = self.fetch(inventory_report, result_dict=True)
        for item in data:
            book_id = item['id']
            borrows, returns = self.borrows_returns_report(book_id)
            temp_dict = {'borrows': borrows, 'returns': returns}
